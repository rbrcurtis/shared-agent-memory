...
describe("GET /api/v1/config", () => {
  it("returns config with valid auth", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json<{
      data: { collectionName: string; modelReady: boolean; qdrantUrl?: string };
    }>();
    expect(body.data.collectionName).toBe(TEST_COLLECTION);
    expect(body.data.modelReady).toBe(true);
    expect(body.data.qdrantUrl).toBeUndefined();
  });
});
...
describe("GET /api/v1/memories/:id/audit", () => {
  it("returns audit events for stored memory", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json<{
      data: Array<{ action: string; actor: string; memoryId: string }>;
    }>();
