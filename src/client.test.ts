...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/v1/config')) {
      return jsonResponse({ data: { qdrantUrl: 'http://qdrant:6333', collectionName: 'shared_agent_memory', modelReady: true } });
    }

    if (url.includes('/api/v1/memories/search')) {
      return jsonResponse({ data: [{ id: 'mem-1', title: 'Memory One', project: 'project-a', score: 0.9 }] });
    }

    if (url.includes('/api/v1/memories/load')) {
      return jsonResponse({ data: [{ id: 'mem-1', title: 'Memory One', project: 'project-a', score: 1 }] });
    }

    if (url.includes('/api/v1/memories/recent')) {
      return jsonResponse({ data: [{ id: 'mem-1', title: 'Memory One', project: 'project-a', created_at: '2026-05-01T00:00:00.000Z' }] });
    }
    ...
    return jsonResponse({ error: { message: 'unexpected request' } }, { status: 500 });
  });
});
...
describe('memory API client', () => {
  ...
  it('passes project filter through when search specifies one', async () => {
    await searchMemory({ query: 'needle', project: 'project-a', tags: ['one', 'two'] });
